...
def render_flexible_analysis(df, category_col, section_title):
    ...
    with col_graph:
        ...
        # --- AÑADIR ESTE BLOQUE ---
        csv_data = convertir_df_a_csv(df_grouped)
        st.download_button(
            label="💾 Descargar datos del gráfico (CSV)",
            data=csv_data,
            file_name=f"datos_{category_col}_{chart_type}.csv",
            mime="text/csv",
            key=f"dl_{category_col}",
        )
...
def render_notebook_style_analysis(df, raw_col, label):
    ...
    with st.expander(
        ...
        with col_viz:
            ...
            # --- AÑADIR ESTE BLOQUE ---
            csv_line = convertir_df_a_csv(df_agrupado)
            st.download_button(
                label=f"💾 Descargar datos Evolución {label} (CSV)",
                data=csv_line,
                file_name=f"evolucion_{raw_col}.csv",
                mime="text/csv",
                key=f"dl_line_{raw_col}",
            )
            ...
            # --- AÑADIR ESTE BLOQUE ---
            csv_bar = convertir_df_a_csv(df_merged)
            st.download_button(
                label=f"💾 Descargar datos Composición {label} (CSV)",
                data=csv_bar,
                file_name=f"composicion_{raw_col}.csv",
                mime="text/csv",
                key=f"dl_bar_{raw_col}",
            )
...
with tab4:
    ...
    # --- AÑADIR ESTE BLOQUE ---
    csv_global = convertir_df_a_csv(df_main)
    st.download_button(
        label="💾 Descargar TODOS los datos filtrados (CSV)",
        data=csv_global,
        file_name="datos_filtrados_global.csv",
        mime="text/csv",
    )
